"""Run the isolated canonical-snapshot Stage 1 formal campaign v4."""
...
ROOT = Path(
    os.environ.get("DYNAMIC_HIERARCHY_PROJECT_ROOT", str(SOURCE_ROOT))
).resolve()
for source_path in (SOURCE_ROOT.resolve(), SOURCE_PACKAGE_ROOT):
...
def _assert_canonical_module_roots() -> None:
    expected_roots = {
        "dynamic_hierarchy": SOURCE_PACKAGE_ROOT,
    ...
    for name, module in sorted(sys.modules.items()):
        expected = next(
            (
                root
                for prefix, root in expected_roots.items()
        ...
        if module_file is None:
            continue
        try:
            Path(module_file).resolve().relative_to(expected)
        except ValueError as error:
            raise RuntimeError(
                f"campaign v4 imported mutable module {name}: {module_file}"
            ) from error


_assert_canonical_module_roots()
...
DEFAULT_CAMPAIGN_ROOT = ROOT / "runs" / "stage1-literal-formal-v4-campaign"
DEFAULT_STATE = ROOT / "runs" / "stage1-literal-formal-v4-sequence.json"
DEFAULT_AGGREGATE = (
    ROOT / "runs" / "stage1-literal-formal-v4-confirmation.json"
)
LEGACY_STATE = ROOT / "runs" / "stage1-literal-formal-sequence.json"
CAMPAIGN_VERSION = "literal-formal-confirmation-v4"
RUN_PREFIX = "stage1-formal-v4-"
...
def _candidate_path(config: Stage1Config) -> Path:
    ...
    if _sha256(path).lower() != (
        ...
        raise SequenceAbort("candidate result digest does not match v4 config")
    return path
...
def _verify_candidate(config: Stage1Config, path: Path) -> dict[str, object]:
    try:
        ...
        raise SequenceAbort(
            f"candidate prerequisite failed for campaign v4: {error}"
        ) from error
    if verification.get("passed") is not True:
        raise SequenceAbort("candidate prerequisite did not pass for campaign v4")
    return verification
...
def _load_state(path: Path) -> dict[str, Any]:
    try:
        ...
        raise SequenceAbort(f"cannot read campaign v4 state: {error}") from error
    if not isinstance(state, dict):
        raise SequenceAbort("campaign v4 state is not an object")
    if (
        ...
        raise SequenceAbort("state is not an isolated campaign v4 state")
    return state
...
def _state_run_roots(state: dict[str, Any]) -> tuple[Path, ...]:
    ...
    for entry in state.get("runs", {}).values():
        ...
        if not run_dir.name.startswith(RUN_PREFIX):
            raise SequenceAbort("campaign state contains a non-v4 run")
        roots.append(run_dir)
    return tuple(roots)


def _launch_receipt_path(state_path: Path, seed: int) -> Path:
    return state_path.with_name(f"{state_path.stem}.seed-{seed}.receipt.json")


def _freshness_excluded_files(
    state_path: Path,
    aggregate_output: Path,
    config: Stage1Config,
) -> tuple[Path, ...]:
    return (
        ...
        aggregate_output,
        *(
            _launch_receipt_path(state_path, seed)
...
def prepare_campaign(
    ...
    if state_path == LEGACY_STATE.resolve():
        raise SequenceAbort("campaign v4 refuses the legacy sequence state path")
    if campaign_root == LEGACY_STATE.resolve().parent:
        raise SequenceAbort("campaign v4 root must be separate from legacy state")

    if state_path.is_file():
        state = _load_state(state_path)
        if Path(state["campaign_root"]).resolve() != campaign_root:
            raise SequenceAbort("campaign root does not match v4 state")
        ...
        if campaign_root.exists():
            ...
            initial_freshness = campaign_seed_freshness(
                ...
                config_path,
                config,
                excluded_files=_freshness_excluded_files(
                    state_path,
                    aggregate_output,
                    config,
                ),
            ...
            if initial_freshness.get("passed") is not True:
                raise SequenceAbort(
                    "campaign v4 seeds are not fresh: "
    ...
    if not _eligible_config(config):
        raise SequenceAbort("frozen campaign config is not formal-v4 eligible")
    candidate_path = _candidate_path(config)
    ...
    if freshness.get("passed") is not True:
        raise SequenceAbort(
            "campaign v4 seed freshness failed: "
    ...
    if not state:
        ...
        for field in (
            ...
            if state.get(field) != expected_state.get(field):
                raise SequenceAbort(f"campaign v4 state mismatch for {field}")
...
def _run_is_live(run_dir: Path) -> bool:
    ...
    if pid_path.is_file():
        try:
            ...
            record = {}
        for value in (record.get("worker_pid"), record.get("pid")):
            if type(value) is not int or not psutil.pid_exists(value):
                continue
            try:
                command = " ".join(psutil.Process(value).cmdline()).lower()
            except (psutil.Error, OSError):
                continue
            if (
                "stage1_worker.py" in command
                and str(run_dir.resolve()).lower() in command
            ):
                return True
...
def _snapshot_launcher(run_dir: Path) -> Path:
    snapshot_root = (run_dir / "snapshot").resolve()
    launcher = (snapshot_root / "scripts" / "start_stage1.ps1").resolve()
    try:
        launcher.relative_to(snapshot_root)
    except ValueError as error:
        raise SequenceAbort("campaign launcher escapes the materialized snapshot") from error
    if not launcher.is_file():
        raise SequenceAbort(f"campaign launcher is missing: {launcher}")
    return launcher
...
def _read_launch_receipt(
    ...
    *,
    ...
    try:
        ...
        raise SequenceAbort(
            f"seed {expected_seed} launch receipt is invalid"
        ) from error
    if (
        Path(str(receipt.get("run_dir", ""))).resolve() != run_dir.resolve()
        or receipt.get("training_seed") != expected_seed
        or receipt.get("prepared_campaign_run") is not True
        or receipt.get("resume") is not expected_resume
        or Path(str(receipt.get("runtime_python", ""))).resolve()
        != Path(sys.executable).resolve()
    ):
        raise SequenceAbort(
            f"seed {expected_seed} canonical launch receipt mismatch"
        )
    return receipt
...
def _launch_prepared(
    ...
    command = [
        ...
        "Bypass",
        "-File",
        str(_snapshot_launcher(run_dir)),
        "-Config",
    ...
    _run_launcher_command(command, receipt_path, f"campaign-v4 seed {seed}")
...
def _resume_prepared(
    ...
    _run_launcher_command(command, receipt_path, f"campaign-v4 resume {run_dir}")
    _read_launch_receipt(
        receipt_path,
        run_dir,
        expected_seed=seed,
...
def _run_campaign(
    ...
    try:
        for index, seed in enumerate(config.confirmation_training_seeds):
            ...
            if not run_dir.name.startswith(RUN_PREFIX):
                raise SequenceAbort("v4 state attempted to use a legacy run")
            result_path = run_dir / "result.json"
            while True:
                manifest = _verify_campaign(campaign_root, config)
                if result_path.is_file():
                    ...
                    if observed.get("state") == "failed":
                        raise SequenceAbort(
                            f"campaign-v4 seed {seed} worker failed: "
                    ...
                    if not all(
                        ...
                        raise SequenceAbort(
                            f"campaign-v4 seed {seed} is not recoverable: {failed}"
                        )
                ...
                receipt_path = _launch_receipt_path(state_path, seed)
    ...
    if len(result_paths) != 8:
        raise SequenceAbort("campaign v4 cannot aggregate without eight results")
...
def run_campaign(
    ...
    if not prepare_only:
        ...
        if SOURCE_ROOT.resolve() != expected_source:
            raise SequenceAbort(
                "campaign v4 execution must use its canonical coordinator"
            )
    mutex = PerRunMutex(ROOT / "runs" / ".stage1-formal-v4-sequence")
    try:
        mutex.acquire()
    except RuntimeError as error:
        raise SequenceAbort(
            "another formal campaign v4 coordinator is active"
        ) from error
    try:
...
def main() -> int:
    ...
    try:
        ...
        print(
            "campaign v4 monitor interrupted; active worker was not stopped. "
