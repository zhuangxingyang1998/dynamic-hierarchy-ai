"""Run the isolated canonical-snapshot Stage 1 formal campaign v3."""
...
DEFAULT_CAMPAIGN_ROOT = ROOT / "runs" / "stage1-literal-formal-v3-campaign"
DEFAULT_STATE = ROOT / "runs" / "stage1-literal-formal-v3-sequence.json"
DEFAULT_AGGREGATE = (
    ROOT / "runs" / "stage1-literal-formal-v3-confirmation.json"
)
LEGACY_STATE = ROOT / "runs" / "stage1-literal-formal-sequence.json"
CAMPAIGN_VERSION = "literal-formal-confirmation-v3"
RUN_PREFIX = "stage1-formal-v3-"
...
def _candidate_path(config: Stage1Config) -> Path:
    ...
    if _sha256(path).lower() != (
        ...
        raise SequenceAbort("candidate result digest does not match v3 config")
    return path
...
def _verify_candidate(config: Stage1Config, path: Path) -> dict[str, object]:
    try:
        ...
        raise SequenceAbort(
            f"candidate prerequisite failed for campaign v3: {error}"
        ) from error
    if verification.get("passed") is not True:
        raise SequenceAbort("candidate prerequisite did not pass for campaign v3")
    return verification
...
def _load_state(path: Path) -> dict[str, Any]:
    try:
        ...
        raise SequenceAbort(f"cannot read campaign v3 state: {error}") from error
    if not isinstance(state, dict):
        raise SequenceAbort("campaign v3 state is not an object")
    if (
        ...
        raise SequenceAbort("state is not an isolated campaign v3 state")
    return state
...
def _state_run_roots(state: dict[str, Any]) -> tuple[Path, ...]:
    ...
    for entry in state.get("runs", {}).values():
        ...
        if not run_dir.name.startswith(RUN_PREFIX):
            raise SequenceAbort("campaign state contains a non-v3 run")
...
def prepare_campaign(
    ...
    if state_path == LEGACY_STATE.resolve():
        raise SequenceAbort("campaign v3 refuses the legacy sequence state path")
    if campaign_root == LEGACY_STATE.resolve().parent:
        raise SequenceAbort("campaign v3 root must be separate from legacy state")

    if state_path.is_file():
        state = _load_state(state_path)
        if Path(state["campaign_root"]).resolve() != campaign_root:
            raise SequenceAbort("campaign root does not match v3 state")
        ...
        if campaign_root.exists():
            ...
            if initial_freshness.get("passed") is not True:
                raise SequenceAbort(
                    "campaign v3 seeds are not fresh: "
    ...
    if not _eligible_config(config):
        raise SequenceAbort("frozen campaign config is not formal-v3 eligible")
    candidate_path = _candidate_path(config)
    ...
    if freshness.get("passed") is not True:
        raise SequenceAbort(
            "campaign v3 seed freshness failed: "
    ...
    if not state:
        ...
        for field in (
            ...
            if state.get(field) != expected_state.get(field):
                raise SequenceAbort(f"campaign v3 state mismatch for {field}")
...
def _launch_prepared(
    ...
    _run_launcher_command(command, receipt_path, f"campaign-v3 seed {seed}")
...
def _resume_prepared(
    ...
    _run_launcher_command(command, receipt_path, f"campaign-v3 resume {run_dir}")
    _read_launch_receipt(
        receipt_path,
        run_dir,
        expected_seed=seed,
...
def _run_campaign(
    ...
    try:
        for index, seed in enumerate(config.confirmation_training_seeds):
            ...
            if not run_dir.name.startswith(RUN_PREFIX):
                raise SequenceAbort("v3 state attempted to use a legacy run")
            result_path = run_dir / "result.json"
            while True:
                manifest = _verify_campaign(campaign_root, config)
                if result_path.is_file():
                    ...
                    if observed.get("state") == "failed":
                        raise SequenceAbort(
                            f"campaign-v3 seed {seed} worker failed: "
                    ...
                    if not all(
                        ...
                        raise SequenceAbort(
                            f"campaign-v3 seed {seed} is not recoverable: {failed}"
                        )
                ...
                if run_dir.exists():
                    ...
                    if _run_is_live(run_dir):
                        ...
                        if {path.name for path in run_dir.iterdir()} != allowed:
                            raise SequenceAbort(
                                "prepared v3 run has partial launch evidence "
                                "without pid.json"
                            )
    ...
    if len(result_paths) != 8:
        raise SequenceAbort("campaign v3 cannot aggregate without eight results")
...
def run_campaign(
    ...
    if not prepare_only:
        ...
        if SOURCE_ROOT.resolve() != expected_source:
            raise SequenceAbort(
                "campaign v3 execution must use its canonical coordinator"
            )
    mutex = PerRunMutex(ROOT / "runs" / ".stage1-formal-v3-sequence")
    try:
        mutex.acquire()
    except RuntimeError as error:
        raise SequenceAbort(
            "another formal campaign v3 coordinator is active"
        ) from error
    try:
...
def main() -> int:
    ...
    try:
        ...
        print(
            "campaign v3 monitor interrupted; active worker was not stopped. "
